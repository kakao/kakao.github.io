...
### 내용
지난 12일 리눅스 재단이 OMI(Open Model Initiative)를 지원하겠다는 발표가 나온 뒤, 이 프로젝트가 윤리적 LLM의 개발을 이끌 수 있을 것이라는 분석이 나오고 있다.<br>
...
## Redis Dual License 적용
- [https://redis.com/blog/redis-adopts-dual-source-available-licensing/](https://redis.com/blog/redis-adopts-dual-source-available-licensing/){:target="_blank" .link}


### 요약
Redis 7.4 버전부터 라이선스가 RSALv2(Redis Source Available License) 또는 SSPLv1(Server Side Public License) 라이선스가 적용됩니다.<br>
이전에 사용되던 BSD-3 라이선스가 더이상 적용되지 않습니다.
{: .h3_box}

### 내용
라이선스 변경 배경
Redis는 처음부터 현대 인터넷을 구동하는 애플리케이션 및 데이터 인프라에 대한 성능과 단순성의 기반을 제공했습니다. 15년이 지난 지금, 우리는 전 세계가 매일 사용하는 실시간 애플리케이션을 지원함으로써 전 세계 수백만 명의 개발자에게 서비스를 제공하게 된 것을 자랑스럽게 생각합니다. 우리는 이미 Redis Stack 배포판에 고급 Redis 모듈에 대한 이중 라이선스를 구현했으며 이는 커뮤니티에서 호평을 받았습니다. 실제로 Redis 6 이상에서 redis.io 다운로드의 50% 이상이 Redis Stack에서 발생합니다. 이제 우리는 이 라이선스를 Redis 자체로 확장함으로써 사용자를 위한 가장 전체적인 데이터 모델, 처리 엔진 및 개발자 기능 세트를 계속해서 발전시킬 수 있다고 믿습니다.
새로운 소스 사용 가능 라이선스를 통해 우리는 소스 코드에 대한 허용된 사용을 지속 가능하게 제공할 수 있습니다. 우리는 클라이언트, 도구, 핵심 Redis 제품 제공의 통합 세트를 갖춘 실시간 데이터 플랫폼으로서 Redis를 다음 개발 단계로 이끌고 있습니다. Redis 소스 코드는 Redis Community Edition을 통해 개발자, 고객 및 파트너가 계속 무료로 사용할 수 있습니다. 향후 Redis 소스 사용 가능 릴리스에서는 검색, JSON, 벡터, 확률 및 시계열 데이터 모델을 포함하는 핵심 Redis를 Redis Stack과 통합하여 다운로드 가능한 소프트웨어로 사용하기 쉬운 무료 패키지로 통합할 것입니다. 이를 통해 누구나 고성능 키/값 및 문서 저장소, 강력한 쿼리 엔진, 생성 AI 애플리케이션을 지원하는 지연 시간이 짧은 벡터 데이터베이스 등 다양한 컨텍스트에서 Redis를 쉽게 사용할 수 있습니다.
{: .h3_box}
Redis의 성공으로 인해 독특한 과제가 발생했습니다. Redis는 기여를 열망하는 역동적인 개발자 커뮤니티와 함께 ​​대량의 개발을 후원해 왔습니다. 그러나 Redis의 상업 판매 대부분은 Redis의 투자와 오픈 소스 커뮤니티를 상품화하는 최대 클라우드 서비스 제공업체를 통해 전달됩니다. 커뮤니티 주도의 거버넌스 모델을 지원하려는 노력과 BSD 라이센스를 유지하려는 우리의 열망에도 불구 하고 오픈 소스, 소스 사용 가능 및 다양한 온프레미스 및 클라우드 플랫폼에 최적화된 상용 소프트웨어 전반에 걸쳐 여러 소프트웨어 배포를 동시에 제공하는 것은 불가능합니다. Redis를 미래로 성공적으로 이끌 수 있는 능력이 부족합니다.
새로운 라이선스에 따라 Redis 제품을 호스팅하는 클라우드 서비스 제공업체는 더 이상 Redis 소스 코드를 무료로 사용할 수 없습니다. 예를 들어, 클라우드 서비스 제공업체는 Redis 코드 유지 관리업체인 Redis와의 라이선스 조건에 동의한 후에만 Redis 7.4를 제공할 수 있습니다. 이러한 계약은 기존 통합 솔루션에 대한 지원을 뒷받침하고 향후 Redis 혁신에 대한 전체 액세스를 제공합니다.
{: .h3_box}
Microsoft의 개발자 부문 사장인 Julia Liuson은 “최신 데이터 스토리지 및 관리 혁신으로 개발자를 지원하기 위한 공동 작업을 계속할 수 있기를 기대합니다.”라고 말했습니다. "우리의 협력은 Azure Cache for Redis와 같은 통합 솔루션을 계속 지원하며 Microsoft 고객에게 Redis 제품 내 확장된 기능에 대한 독점적인 액세스를 제공할 것입니다."
실제로 이중 라이선스에 따라 허용 라이선스를 계속 누리게 될 Redis 개발자 커뮤니티에는 아무런 변화가 없습니다. 동시에 Redis가 책임지는 모든 Redis 클라이언트 라이브러리는 오픈 소스 라이선스를 유지합니다. Redis는 파트너 프로그램을 통해 Redis가 개발하고 제공하는 모든 향후 릴리스, 업데이트 및 기능에 독점적으로 액세스하여 관리 서비스 제공업체 및 시스템 통합업체를 포함한 광범위한 파트너 생태계를 계속 지원할 것입니다 . 기존 Redis Enterprise 고객에게는 변경 사항이 없습니다.
우리의 새로운 라이선스 접근 방식은 Redis 소스 코드를 광범위하게 사용 가능하게 하고, 최소한의 제한으로 개발자 커뮤니티를 지원하며, 기능이 풍부한 무료 소프트웨어 및 엔터프라이즈 제품에 대한 지속적인 투자 능력을 보호하는 것 사이에서 최상의 균형을 유지합니다.
항상 그랬듯이 우리 팀, 커뮤니티, 고객 및 파트너는 Redis를 선도적인 실시간 데이터 플랫폼으로 생성, 발전 및 배포하는 데 계속해서 앞장설 것입니다.
{: .h3_box}

#### 참고
* [RSALv2(Redis Source Available License)](https://redis.com/legal/rsalv2-agreement/){:target="_blank" .link} 특징 
* 사용/배포등은 자유롭지만 Redis 이외 다른 클라우드 등에서 서비스하는 것을 제한하는 형태의 라이선스
  * 라이선스 제공자는 귀하에게 소프트웨어의 파생물을 사용, 복사, 배포, 제공 및 준비할 수 있는 비독점적이고 로열티가 없으며 전 세계적으로 재라이센스가 불가능하고 양도 불가능한 라이센스를 부여
  * 귀하는 소프트웨어 또는 수정된 버전의 기능을 제3자에게 서비스로 제공할 수 없으며, 소프트웨어의 기능을 제3자가 사용할 수 있도록 하는 방식으로 소프트웨어 또는 수정된 버전을 배포할 수 없습니다.
{: .fs-4 .h4_box }
...
11. 원래의 3절 BSD 라이센스에 따라 제공된 제품 버전을 계속 사용할 수 있습니까?
    예. 라이센스 변경은 소급 적용되지 않습니다. 이는 변경 이전의 모든 소스 코드와 릴리스가 3절 BSD 라이센스에 따라 유지된다는 것을 의미합니다. 귀하는 해당 이용 약관을 준수하는 한 원래 라이센스에 따라 해당 버전을 무기한으로 계속 사용할 수 있습니다. Redis는 현재 보안 정책 에 따라 Redis Community Edition이 출시될 때까지 계속해서 보안 업데이트를 제공하고 이러한 릴리스 내에서 기타 중요한 결함을 해결할 계획입니다 .

12. Redis는 3절 BSD 라이센스에 따라 보안 패치를 이전 릴리스로 백포트합니까?
    Redis는 현재 보안 정책 에 따라 Redis Community Edition 9.0이 출시될 때까지 사용 가능한 중요한 보안 패치를 3항 라이선스에 따라 기존 버전으로 계속 백포트할 것입니다 . 해당 날짜 이후의 모든 패치는 새로운 이중 라이센스에 따라 제공됩니다.
