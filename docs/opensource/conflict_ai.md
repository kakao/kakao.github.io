...
* **입력 리스크(Input Risk)**  
  AI 모델이 학습하는 데이터는 다양한 라이선스 조건(또는 라이선스 조건 없이)에 따라 제공되고 각각 저작권 침해 위험 요소가 상이하기 때문에, 모델의 학습에 어떤 데이터를 사용할지 결정하는 것이 어렵습니다. 이러한 상황은 다수의 AI 개발자가 마주하는 문제이기도 합니다. <br>
