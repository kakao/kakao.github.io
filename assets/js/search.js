...
function populateResultContent(html, item) {
    ...
    html = injectContent(html, item.author.name, '##AuthorName##');
    html = injectContent(html, item.author.image, '##AuthorImage##');
    html = injectContent(html, item.author.url, '##AuthorUrl##');

    var tagHtml = '';

    for (var i = 0; i < item.tags.length; i++) {
        var tag = item.tags[i];
        tagHtml += '<a href="' + item.baseurl + '/tags/' + tag + '" title="' + tag + '" class="tag tag-' + tag + '">' + tag + '</a>';
    }

    html = injectContent(html, tagHtml, '##Tags##');
